...
const ManageClass = () => {
  ...
  return (
    <div>
      ...
      {classes.map((c) => (
        <div className='flex justify-between rounded items-center border border-lime-600' key={c._id}>
          <div className='ml-2 md:ml-0'>
          <figure className="hidden md:block">
            <img className="md:h-[250px] w-[250px] object-cover rounded" src={c.image} alt={c.name} />
          </figure>
          ...
          <p>Email: {c.email}</p>
          <p className='mt-4'>Status: <span className=' text-blue-700'>{c.status}</span></p>
          </div>
